...
const getForecast = () => {
    let inputUser = inputField.value
    const urlToFetch = `${forecastUrl}${apiKey}&query=${inputUser}`
     fetch(urlToFetch).then(res => res.json()).then(data => createWeatherHTML(data)).catch(err => console.log(err))
}
